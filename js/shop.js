$(document).ready(function() {
	...
	function addRow() {
		...
		/*	document.querySelector('.cart-body').insertAdjacentHTML(
			...
			$('.product0').val("Nike Футболка NRG ACG Hike Box Tee");
			$('.amount0').val(cart[i]);
					break;
			...
			$('.product1').val("Nike Кеды AIR FORCE 1 07 LE");
			$('.amount1').val(cart[i]);
					break;
			...
			$('.product2').val("Nike Кеды AIR FORCE 1 LOW RETRO");
			$('.amount2').val(cart[i]);
					break;
			...
			$('.product3').val("Nike Кеды AIR FORCE 1 LOW RETRO");
			$('.amount3').val(cart[i]);
					break;
			...
			$('.product4').val("Nike Ветровка M NK WINDRNNER JKT DYE");
			$('.amount4').val(cart[i]);
					break;
